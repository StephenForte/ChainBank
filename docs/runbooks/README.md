...
| --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| No credential revoke/disable API, script, or repository write method        | Cannot disable `api_credentials` through the product                                                                                              | SQL `UPDATE` on `enabled` / `revoked_at` (runbooks 3 and 6)                                                      |
...
| Credential disable via SQL writes no audit event                            | Authorization-relevant change is not recorded in `audit_events`                                                                                   | Note the change in your incident log; product audit path is a follow-up                                          |
