...
| `SIGNER_CHAIN_MISMATCH`                      | 503  | RPC chain id ≠ configured `CHAIN_ID`                                                                          | Fix that chain's `rpcUrl` and `chainId` inside the dashboard `CHAINS` document; do not set the singular `CHAIN_ID` or `CHAIN_RPC_URL` env keys beside `CHAINS`. Refuse to sign until they match.                                                     |
