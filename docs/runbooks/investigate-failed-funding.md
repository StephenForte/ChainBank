...
| -------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FUNDING_DISABLED`                           | 403  | `FUNDING_ENABLED=false` or `FUNDING_KILL_SWITCH=true` (public text differs: disabled vs temporarily disabled) | Check Render env on `chainbank-web`. Kill switch / enable flips need **redeploy/restart** to take effect.                                                                               |
...
| `SIGNER_UNAVAILABLE`                         | 503  | Funding enabled but signer missing/unusable in this process                                                   | Confirm `TREASURY_PRIVATE_KEY` on **web** only, structurally valid, and `FUNDING_ENABLED=true` for a signing-capable role.                                                              |
