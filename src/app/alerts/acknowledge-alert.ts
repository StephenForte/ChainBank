...
import type { OperatorMutationTransaction, StoredAlert } from '../ports.js';
 ...
 *
 * The acknowledgement write and its audit entry commit atomically (C21).
 */
export async function acknowledgeAlert(
  ...
  return dependencies.operatorMutations.run(async (uow) => {
    const existing = await uow.alerts.findById(input.alertId);
