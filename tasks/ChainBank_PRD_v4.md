...
| 4 — Managed-wallet reconciliation      | **In progress — held open pending TX.10.** Merged and deployed: the reconciliation use case ([C14](./DECISIONS.md#c14--reconciliation-use-case-owner-t41), migration `0004`) — below-minimum sweep through the C7 dispatch engine, evidence-based `submission_unknown` settlement, crash-orphan outgoing scan; the 6-hourly `chainbank-wallet-reconciler` cron (T4.2); consecutive-failure alerting (T4.3, [C15](./DECISIONS.md#c15--reconciliation-failure-alert-owner-t43)); the incremental outgoing scan (TX.9, migration `0005`) — forward-contiguous windows from a per-treasury watermark, with the `submission_unknown` nonce hunt bisecting on account nonce instead of sweeping blocks; and cron-vs-API concurrency tests (T4.4, C16). **Remaining: TX.10.** T4.4 measured a crash-induced duplicate transfer under the cron-vs-API race (two transfers broadcast, one recorded), so the §20 exit criterion "API and cron concurrency tests pass" is met for lock serialization, nonce discipline, reserve behaviour, and watermark integrity but **not** for crash-induced duplicates. The duplicate is detected by TX.9's outgoing scan; prevention lands in TX.10, and the phase exits then. |
...
- **Concurrent crash-induced duplicate transfers are detected but not
  prevented — Phase 4 is held open for this (TX.10).** T4.4 measured it: killing
  the lock-holding backend mid-send while an API caller waits on the advisory
  lock broadcasts **two** transfers while recording **one**
  `funding_transactions` row. The killed backend's in-lock rows roll back, so the
  first send leaves no trace and nothing gates the waiter, which re-reads a
  not-yet-mined balance and sends again. TX.9's outgoing scan surfaces the
  orphan as a critical `unexplained_outgoing_transfer`, so the money is never
  silently lost — but §20's P4-US2 requires that concurrent jobs and API calls do
  not _issue_ duplicate transactions, and detection is not prevention.
