...
export type ReconciliationPanelProps = {
  readonly loadReconciliationRuns: () => Promise<void>;
  readonly loadFindingAlerts: () => Promise<void>;
...
*/}
                    ...
                    <article key={alert.id} className="finding finding-acknowledged">
                      ...
                      <dl className="facts">
                        ...
                        <div>
                          <dt>Acknowledged</dt>
                          <dd>
                            {alert.acknowledgedAt === null ? '—' : dash.formatTimestamp(alert.acknowledgedAt)}
        ...
        {reconciliationState === 'empty' ? (
          <p className="muted">No reconciliation runs returned ({String(reconciliationRunsTotal)} total).</p>
        ) : null}
          ...
          ? (() => {
                ...
                <>
                  <dl className="facts">
                    ...
                    <div>
                      <dt>Value</dt>
                      <dd className="mono">
                        {finding.valueWei === undefined ? '—' : dash.formatFindingWei(finding.valueWei)}
                      </dd>
                    </div>
                    <div>
                      <dt>Nonce</dt>
                      <dd className="mono">{finding.nonce === undefined ? '—' : String(finding.nonce)}</dd>
                    </div>
                    <div>
                      <dt>Block</dt>
              ...
              const renderUnacknowledgedCriticalFinding = (finding: FindingView, index: number) => {
                ...
                const canAcknowledge =
                  canAcknowledgeFindings && dash.findingAlertEntityId(finding) !== undefined;
                const href = dash.explorerTxUrl(treasuries, finding.treasuryId, finding.transactionHash);
                const displayId =
                ...
                return (
                  ...
                  >
                    ...
                    {canAcknowledgeFindings && canAcknowledge ? (
                    ...
                    {ackError !== undefined ? <p className="error-inline">{ackError}</p> : null}
                    {isExpanded ? renderCriticalFindingDetail(finding, displayId, href, undefined) : null}
                  </article>
                );
              };
              ...
              return (
                <>
                  <div
                    className={summaryNeedsAttention ? 'recon-summary recon-summary-alert' : 'recon-summary'}
                  >
          ...
          */}
                  ...
                  >
                    <p className="muted">
                      ...
                      (newest first). Findings below are from this page only — older critical findings outside
                      the window will not appear here.
                    </p>

                    {!hasCritical ? <p className="muted">No critical findings in the loaded runs.</p> : null}

                    {acknowledgedCriticalFindings.length > 0 ? (
                      <>
                        <h3 className="subsection-title">Acknowledged critical findings</h3>
                        <p className="muted">
                          Acknowledged findings stay in the run record with their note — collapsing hides them
                          from the always-visible block only after an operator stands them down (C20 / TX.20).
                        </p>
                        <div className="finding-list">
                        ...
                        {warningFindings.map((finding, index) => (
                          ...
                          >
                            <div className="finding-head">
                              <span
                                className={
                                  finding.severity === 'warning' ? 'badge badge-warn' : 'badge badge-unknown'
                                }
                              >
                                {finding.severity}
                              </span>
                            ...
                            <p className="muted">
                              Run <code>{finding.runId}</code> · {dash.formatTimestamp(finding.runStartedAt)}
